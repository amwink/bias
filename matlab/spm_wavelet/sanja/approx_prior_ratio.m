%------------------------------------------------------------------------------------
% Function: approx_prior_ratio.m
% 
% Piece-wise linear fitting of the log-likelihood ratio for the local spatial 
% activity indicator using its empirical conditional densities according to [1].
% 
%
% Input: 
%       M - the detected edge mask for the given detail image
%       D - the input detail image 
%       Dcoarser - the corresponding detail image at the next coarser scale
%       W - a parameter which describes the window size: w=(window size-1)/2
...
function [arg,ratio,confid] = approx_prior_ratio(M,D,Dcoarser,W)
...
h=NeighbEn_sector(M,D,Dcoarser,W);
%step=round(Max/40);
step=4;
x=0:step:Max;
...
h=NeighbEn_sector(1-M,D,Dcoarser,W);
pdf_noise=hist(h,x);
pdf_noise(1)=0;
...
%title('Averaged energy likelihood');
