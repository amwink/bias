function ws = MultiVisu2(wc,type,L)
%
...
for j=J-1:-1:L
    ...
    case 'SOFT'
         ws(dyad(j),dyad(j)) = SoftThreshAbs(ws(dyad(j),dyad(j)),thr) ;
         ws(bdyad(j),dyad(j)) = SoftThreshAbs(ws(bdyad(j),dyad(j)),thr) ;
         ws(dyad(j),bdyad(j)) = SoftThreshAbs(ws(dyad(j),bdyad(j)),thr) ;
    end;
end
