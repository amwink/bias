...
write_map(inputfile, M, msk, vcurr, 'fastECM', sprintf('ECM [iterations: %d]',iter) );
...
if ( ( (nargin>1) & (rankmap ~= 0) ) | ( (nargin>2) & (normmap~=0) ) )
  ...
  if (rankmap ~= 0) 
    write_map(inputfile, M, msk, rvcurr, 'rankECM', 'ECM [converted to <0,1> ranks]');
  end
  ...
  if ( (nargin>2) & (normmap ~= 0) )
    ...
    write_map(inputfile, M, msk, nvcurr, 'normECM', 'ECM [converted to N(0,1) values]');
...
function write_map(inputfile, M, msk, vcurr, mapfile, descrip)
%
